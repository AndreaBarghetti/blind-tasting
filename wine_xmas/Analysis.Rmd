...
In genere, siamo convinti che il vino piú costa piú ci piacerá.
Nel nostro caso i dati confermano un pó questa convinzione: c'e una correlazione positiva tra prezzo reale e giudizio del vino.
...
Il punteggio viene é determinato dalla media degli scarti tra il prezzo reale e quello indovinato (cioé vince chi ha fatto meno punti)
...
Ogni vino viene percipito piú o meno forte, giustamente, e ognuno traduce il grado percepito con una scala diversa.
...
ethobyname <- rating %>%
  ...
  ggtitle("Grado percepito per sommelier")
...
ethobywine <- rating %>%
  ...
  ggtitle("Grado percepito per vino")
...
Come sono stati apprezzate le bottiglie in gara? E i vini (visto che 2 bottiglie erano identiche)
...
```{r}
rating %>%
  ggplot(aes(y=grade, x=wine_name)) +
  geom_violin() +
  geom_point(position=position_dodge2(width=.2), shape=21, alpha=.5, col="black", size=2) +
  geom_point(size=2,shape=23, stat="summary", fill="red") +
  theme_classic() +
  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1),
        text=element_text(size=12)) +
  xlab("") +
  ggtitle("Giudizi sul vino, per vino")
```
...
## Il vincitore indiscusso: `r winner_wine`` 
...
Questo invece illustra come ognuno é stato piú o meno largo di giudizi. C'é chi apprezza tutto, chi non si sbilancia mai, chi non giudica mai troppo duramente, ecc.
